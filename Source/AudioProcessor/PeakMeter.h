...
class PeakMeter
{
  public:
    ...
    void prepare(const juce::dsp::ProcessSpec &spec)
    {
        ...
        buffer = juce::dsp::AudioBlock<float>(channelPtrs.data(), (size_t)numChannels, (size_t)numSamples);
    ...
    template <typename T> void push(juce::dsp::AudioBlock<T> block)
    {
        jassert(block.getNumChannels() == buffer.getNumChannels());

        // 내부 버퍼 길이보다 긴 경우 뒤쪽 구간만 사용
        const auto trimmed =
            block.getSubBlock(block.getNumSamples() - std::min(block.getNumSamples(), buffer.getNumSamples()));
        ...
        for (auto samplesRemaining = (int64)trimmed.getNumSamples(); samplesRemaining > 0;)
        {
            ...
            const auto srcBlock =
                trimmed.getSubBlock(trimmed.getNumSamples() - (size_t)samplesRemaining, (size_t)numSamplesToWrite);
