...
template <typename SampleType> class TransientNoiseProcessor : public juce::dsp::ProcessorBase
{
  public:
    TransientNoiseProcessor() : transientFollower(), bitCrusher(), airLayer() {}
    ...
    void process(const juce::dsp::ProcessContextReplacing<SampleType> &context) override
    {
        ...
        if (sidechainBPFOn || sidechainListen)
        {
            ...
            sidechainBPFGain.setGainDecibels(
                skewedMap(sidechainBPFFreq, 50.0f, 12000.0f, -6.0f, 12.0f, 0.12f)); // 0.27f
