...
template <typename SampleType> class BandPassFilter : public juce::dsp::ProcessorBase {
public:
  BandPassFilter() {}
  ...
  void reset() override { bandPass.reset(); }
  ...
  // 파라미터 설정 (목표값)
  void setFrequency(float newFreqHz) { frequency = juce::jlimit(20.0f, (float)(sampleRate * 0.45), newFreqHz); }

  void setQ(float newQ) { q = juce::jmax(0.1f, newQ); }
...
private:
  void smoothCoefficients() {
    ...
    auto newCoeffs = juce::dsp::IIR::Coefficients<SampleType>::makeBandPass(sampleRate, currentFrequency, currentQ);
  ...
  juce::dsp::ProcessorDuplicator<juce::dsp::IIR::Filter<SampleType>, juce::dsp::IIR::Coefficients<SampleType>> bandPass;
