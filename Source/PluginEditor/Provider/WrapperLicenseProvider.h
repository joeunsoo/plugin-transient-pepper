...
struct WrapperLicenseProvider : LicenseProvider
{
    ...
    // 네트워크 요청
    std::pair<int, juce::String> sendActivationRequest(juce::String userEmail, juce::String userPassword) override
    {
        return wrapper.licenseManager.sendActivationRequest(userEmail, userPassword);
    }

    // 트라이얼
    int64 getTrial() const noexcept override { return wrapper.licenseManager.getTrial(); }
    int getTrialDays() const noexcept override { return wrapper.licenseManager.getTrialDays(); }
    int64 startTrial() override { return wrapper.licenseManager.startTrial(); }
};
