...
void AboutModal::disableAllChildrenMouse()
{
    for (int i = 0; i < getNumChildComponents(); ++i)
    {
        auto* c = getChildComponent(i);
        if (c == &flexContainer)
            continue; // flexContainer는 이벤트 허용
        c->setInterceptsMouseClicks(false, false);
    }

    // flexContainer 내부는 선택적으로 label 클릭 무시
    for (int i = 0; i < flexContainer.getNumChildComponents(); ++i)
        flexContainer.getChildComponent(i)->setInterceptsMouseClicks(false, false);
}
