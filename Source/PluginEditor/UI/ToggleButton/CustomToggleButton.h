...
struct CustomToggleButton : public juce::TextButton {
  CustomToggleButton(const ScaleProvider &sp) : juce::TextButton(), scaleProvider(sp) { setClickingTogglesState(true); }
  ...
  void paintButton(juce::Graphics &g, bool shouldDrawButtonAsHighlighted, bool shouldDrawButtonAsDown) override {
    ...
    laf.drawButtonBackground(g, *this, backgroundColor, shouldDrawButtonAsHighlighted, shouldDrawButtonAsDown);
    ...
    if (svgDrawable != nullptr) {
      ...
      svgToDraw->drawWithin(g, bounds, juce::RectanglePlacement::centred, 1.0f * scale);
    } else {
      laf.drawButtonText(g, *this, shouldDrawButtonAsHighlighted, shouldDrawButtonAsDown);
    }
  }

  void setSvgDrawable(std::unique_ptr<Drawable> drawable) { svgDrawable = std::move(drawable); }
