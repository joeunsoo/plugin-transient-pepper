...
GraphContainer::GraphContainer(const ScaleProvider &sp, ProcessorProvider &pp)
    : scaleProvider(sp), processorProvider(pp), inputLevelGraph(sp, DARK_RGB_9, DARK_RGB_2),
...
void GraphContainer::paint(juce::Graphics &g)
{
    ...
    juce::DropShadow ds(juce::Colours::black.withAlpha(0.5f), int(3 * scale), {0, int(2 * scale)});
