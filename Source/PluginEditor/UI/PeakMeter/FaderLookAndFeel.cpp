...
void FaderLookAndFeel::drawLinearSlider(juce::Graphics &g, int x, int y, int width, int height, float sliderPos,
  ...
  g.fillRoundedRectangle({cx - thumbWidth * 0.5f, cy - thumbHeight * 0.5f, thumbWidth, thumbHeight},
                         thumbHeight * 0.5f);
  ...
  if (!isValueText) {
    ...
    g.drawLine(cx - lineWidth * 0.5f, cy - 2.0f * scale, cx + lineWidth * 0.5f, cy - 2.0f * scale);
    g.drawLine(cx - lineWidth * 0.5f, cy + 2.0f * scale, cx + lineWidth * 0.5f, cy + 2.0f * scale);
  } else {
    // 값 텍스트 준비
