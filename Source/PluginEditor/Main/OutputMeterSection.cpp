...
OutputMeterSection::OutputMeterSection(Providers &pv)
    : scaleProvider(pv.scale), outputPeakMeter(pv.processor, 0, true), outputMeterTick(pv.scale, false, false),
...
void OutputMeterSection::resized()
{
    ...
    tickBox.items.add(FlexItem(outputMeterTick).withFlex(1.0f).withMargin({0, 0, 0, 18 * scale}));

    tickBox.items.add(FlexItem(meterTickLabel).withWidth(18.0f * scale).withFlex(0.0f, 0.0f));
