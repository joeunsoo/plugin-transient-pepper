...
struct CustomLookAndFeel : public LookAndFeel_V4
{
  public:
                              ...
                              bool shouldDrawButtonAsHighlighted, bool shouldDrawButtonAsDown) override
    {
        ...
        juce::DropShadow ds(juce::Colours::black.withAlpha(0.5f), int(2 * scale), {0, int(3.0f * scale)});
        ...
        juce::ColourGradient gradient(button.getToggleState() ? SECONDARY_DARK_RGB_0  // 활성 (ON)
                          ...
                          float rotaryEndAngle, Slider &slider) override
    {
...

        {
...

            {
                ...
                Path outlineArc;
                outlineArc.addCentredArc(rx + rw * 0.5f, // 중심 X
                                         ry + rw * 0.5f, // 중심 Y
                                         rw * 0.5f,      // X 반지름
                                         rw * 0.5f,      // Y 반지름
                                         0.0f,           // rotation
            ...
            auto drawDot = [&](float rad)
            {
                ...
                juce::Rectangle<float> dotArea(startX - dotRadius, startY - dotRadius, dotRadius * 2, dotRadius * 2);
...

        {
            ...
            // 각도 계산
            float angle =
                juce::degreesToRadians(180.0f) + rotaryStartAngle + sliderPos * (rotaryEndAngle - rotaryStartAngle);
