...
MixComponent::MixComponent(Providers &pv)
    : scaleProvider(pv.scale), processorProvider(pv.processor),
      ...
      noisePeakMeter(pv.processor, 2, true), outputPeakMeter(pv)
{
...
void MixComponent::parameterChanged(const juce::String &, float)
{
    ...
    bool sidechainListen =
        (processorProvider.state().getRawParameterValue(ID::sidechainListen.getParamID())->load() >= 0.5f);
...
void MixComponent::resized()
{
    ...
    float NoiseGainGap = 11.0f;
    auto area = getLocalBounds().withTrimmedBottom(int(12 * scale));
    auto leftArea = area.removeFromLeft(area.getWidth() / 2);
    noisePeakMeter.setBounds(leftArea.removeFromTop(
        leftArea.getHeight() - int((UI_KNOB_HEIGHT + NoiseGainGap + UI_BUTTON_HEIGHT + UI_KNOB_HEIGHT) * scale)));
